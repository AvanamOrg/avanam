...
class BuilderComponent extends Component {
	...
	focusPanel( item ) {
		if ( undefined !== this.props.customizer.section( 'base_customizer_' + item ) ) {
			this.props.customizer.section( 'base_customizer_' + item ).focus();
		}
	}
	focusItem( item ) {
	...
	render() {
		return (
			<div className={ `base-control-field base-builder-items${ ( this.controlParams.rows.includes( 'popup' ) ? ' base-builder-items-with-popup' : '' ) }` }>
				...
				<div className="base-builder-row-items">
	...
	// linkFocusButtons() {
	// 	this.props.control.container.on( 'click', '.base-builder-areas .base-builder-item', function( e ) {
	// 		e.preventDefault();
